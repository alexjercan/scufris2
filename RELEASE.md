...
9. Verify every workflow the release push started, not only `release`. Pushing `master` also starts `check`, `Documentation`, and `iOS`, and all of them run against the tagged commit. `Documentation` and `iOS` are path-filtered, so a version commit that touches neither `docs/`, `nix/`, nor `surfaces/ios/` starts only `check`; read the last commit that did touch them instead of dispatching those workflows on the tag. Never dispatch `Documentation` on a tag: GitHub Pages deploys from `master` alone, so its `deploy` job is rejected by environment protection and the run goes red for a reason that is not a defect. The Swift suite runs in `iOS` and nowhere else: it needs Xcode, so no step above reaches it and CI is its only gate. `gh run list --limit 5` after the push shows every one.
