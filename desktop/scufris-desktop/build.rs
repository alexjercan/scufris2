...
///
/// A backend is a directory under `desktop/backends/` holding `backend.py`, and
...
fn build_backends() {
    ...
    let mut directories: Vec<String> = fs::read_dir(&backends)
        .expect("desktop/backends is missing")
        .filter_map(|entry| {
            let entry = entry.expect("desktop/backends could not be read");
            entry
                .path()
                .join("backend.py")
                .is_file()
    ...
    let root = env::current_dir().expect("the build has a working directory");
    let mut table = String::from(
        "// Generated by build.rs from desktop/backends. Do not edit.\n\
