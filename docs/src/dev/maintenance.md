...
- `tests/*.test.ts`: extension behavior in Node with the Pi APIs stubbed:
  orchestration, response shaping, Calm, identity, and repository structure. `tests/service.test.ts` covers the agent side of protocol v9
...
- `Cargo.toml`: the root Rust workspace. `shared/control/` owns the protocol encoding,
  ...
  the session, and the three protocol v9 sockets. Every port is faked and the service's stand-in
