...
| Method       | Path                    | Purpose                                          |
| ------------ | ----------------------- | ------------------------------------------------ |
| `GET`        | `/` or `/surface`       | Upgrade to a protocol-v11 surface WebSocket      |
...
it only through an ordinary protocol-v11 `surface.message` after confirmation.
...
sequenceDiagram
    ...
    Service-->>Client: surface.briefings
...
```json
{"v":11,"type":"surface.hello","surface":{"id":"laptop-a","name":"Laptop A","widgets":[]}}
{"v":11,"type":"surface.message","id":"message-1","text":"What changed?","attachments":["att_opaque"]}
{"v":11,"type":"surface.abort","id":"abort-1"}
{"v":11,"type":"job.command","id":"3f81c204b1e9","action":"cancel"}
{"v":11,"type":"briefing.dismiss","id":"generation-a"}
{"v":11,"type":"offer.take","id":"offer-a1"}
```
...
| Briefing rows               | 16 generations                 |
| Briefing summary            | 256 bytes                      |
