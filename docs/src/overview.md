...
a tray icon, built from the same workspace and shipped as a separate Linux
package. It is a client of the service. It records, transcribes locally,
submits the words as an ordinary user message, and speaks whatever paragraph
the agent asks it to. The conversation is the service's, so a companion crash
never stops it and a machine with no screen still has one. See
