...
#[utoipa::path(
    ...
    security(("bearer_token" = [])),
    responses(
        (status = 101, description = "Upgrade to the strict protocol-v11 surface WebSocket"),
        (status = 401, description = "Missing or invalid bearer token", body = ErrorEnvelope)
    )
)]
...
mod tests {
    ...
    fn websocket_payloads_use_the_strict_surface_decoder() {
        let request = decode_request(
            r#"{"v":11,"type":"surface.message","id":"ios-1","text":"hello","attachments":[]}"#,
        )
        .unwrap();
        ...
        assert!(
            decode_request(
                r#"{"v":11,"type":"control.wake","id":"wake-1","custom_type":"scufris-wake","text":"Wake up."}"#
            )
            .is_err()
        );
    }
    ...
    #[tokio::test]
    async fn authenticated_websocket_still_bridges_strict_surface_v11() {
        let root =
