...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../scufris-widgets/widgets");
    println!("cargo:rerun-if-changed=../scufris-widgets/backends");
    build_frontend();
    build_widgets();
    build_backends();
    tauri_build::build();
...
/// Compiles every widget under native/scufris-widgets/widgets and writes the
...
fn build_widgets() {
    let widgets = Path::new("..").join("scufris-widgets").join("widgets");
    ...
    let mut directories: Vec<String> = fs::read_dir(&widgets)
        .expect("native/scufris-widgets/widgets is missing")
        .filter_map(|entry| {
            let entry = entry.expect("native/scufris-widgets/widgets could not be read");
            entry
                .path()
                .join("widget.toml")
                .is_file()
    ...
    let mut table = String::from(
        "// Generated by build.rs from native/scufris-widgets/widgets. Do not edit.\n\
         pub const INSTALLED: &[Source<'static>] = &[\n",
    );
    for directory in directories {
        let manifest = root
            .join("../scufris-widgets/widgets")
            .join(&directory)
            .join("widget.toml");
        ...
        let script = root
            .join("../scufris-widgets/widgets/dist")
            .join(&directory)
            .join("widget.js");
        assert!(
            script.is_file(),
...
///
/// A backend is a directory under `native/scufris-widgets/backends/` holding
...
fn build_backends() {
    let backends = Path::new("..").join("scufris-widgets").join("backends");
    let mut directories: Vec<String> = fs::read_dir(&backends)
        .expect("native/scufris-widgets/backends is missing")
        .filter_map(|entry| {
            let entry = entry.expect("native/scufris-widgets/backends could not be read");
            entry
                .path()
                .join("backend.py")
                .is_file()
    ...
    let root = env::current_dir().expect("the build has a working directory");
    let mut table = String::from(
        "// Generated by build.rs from native/scufris-widgets/backends. Do not edit.\n\
         const INSTALLED: &[Backend<'static>] = &[\n",
    );
    for directory in directories {
        let script = root
            .join("../scufris-widgets/backends")
            .join(&directory)
            .join("backend.py");
        table.push_str(&format!(
            "    Backend {{\n        id: {directory:?},\n        script: include_str!({:?}),\n    }},\n",
