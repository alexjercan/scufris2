// How much of the Codex subscription is spent, and how long until it is not.
//
// The headline is the window closest to biting rather than a fixed one: a
// five hour window at 90 percent is what stops the next question, and a weekly
// figure beside it would be the calmer of the two numbers and the wrong one to
// read first. Every window is listed under it, so the headline is never the
// whole answer.
//
// Its twin is the `claude` widget. They read the same shape and draw the same
