...
- `review-ready`: Scufris runs the declared independent preflight, then opens Plannotator only after exact preflight approval. Do not open another review or spawn a reviewer.
- Preflight findings and Plannotator feedback return automatically to the same worker. A third preflight change request blocks for Pair mediation. Human Plannotator approval still requires the worker's exact `done` acknowledgment before guarded local landing.
