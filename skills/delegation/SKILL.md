...
- `review-ready`: Scufris announces an input-capable `preflight-<review_id>` window in the worker's feature tmux session and runs the actual independent reviewer in Pi's interactive TUI there. The user can watch the prompt, tool activity, progress, and result. Scufris opens Plannotator only after exact preflight approval. Do not select or steer the window, expose reviewer control, open another review, or spawn a reviewer.
