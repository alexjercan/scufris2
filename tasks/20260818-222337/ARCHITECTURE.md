...
Every landable delegated job selects one preflight profile at spawn: code, consumer, operations, or interface. A non-landable result selects none. After `review-ready`, Scufris starts a fresh headless Pi reviewer with Sol at medium thinking. The private helper owns the exact non-tmux child, dedicated session directory, read-only built-in tool allowlist, bounded patch and output, environment isolation, an exact 1800-second reviewer deadline, and pre/post snapshot checks. The helper signals when the exact reviewer child starts. This resets the extension deadline to 1810 seconds and gives the helper a fixed 10-second shutdown margin beyond reviewer execution. It owns review policy, feedback cycles, revision-bound approval, Plannotator ordering, and shutdown cancellation.
