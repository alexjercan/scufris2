...
Speech captures the configured mode and branch-entry boundary at `agent_start`, then extracts a safe final response added after that boundary at `agent_settled`. This covers ordinary prompts and extension-triggered wake runs, including job lifecycle and review feedback messages. The settlement guard produces one playback after retries and queued continuations. A run with no new safe response cannot replay older speech. Raw wake messages, intermediate output, and unsafe responses never enter playback.
...
After the normal review snapshot passes, Scufris creates a detached `preflight-<review_id>` window in the recorded exact feature session. It keeps pane input enabled, enables remain-on-exit, verifies the exact session, window, pane, launcher PID, and reviewer PID, and starts Pi inside that pane. It never attaches, selects, or switches a client. Pi uses its regular interactive TUI, model `openai-codex/gpt-5.6-sol`, medium thinking, the read-only `read`, `grep`, `find`, and `ls` tools, one private `submit_preflight` result tool, a dedicated session directory, disabled extension discovery, disabled skills, disabled prompt templates, disabled themes, and disabled project trust. The trusted private extension writes the result tool arguments to the bounded scratch result file and requests graceful Pi shutdown. One reviewer execution has an exact 1800-second deadline that starts immediately after child creation. After exact-child interruption handling is installed, the pane launcher writes exact readiness evidence. The parent helper validates that evidence and emits one private readiness line. Receipt resets the enclosing extension deadline to 1810 seconds. The helper therefore has a fixed 10-second margin to stop the exact child and return its fail-closed response. Correction review respawns the exact same dead pane and uses the exact saved reviewer session file. Scufris strips implementation-session and Scufris role variables from the reviewer environment. The reviewer receives:
...
The reviewer does not receive the implementation transcript, worker report, reasoning, or claims. The TUI shows the review prompt, progress, tool activity, and final state. The pane can technically receive user input, but Scufris remains the only review owner and result consumer. A separate bounded result file carries the submitted tool arguments to strict JSON validation; pane capture is never parsed. It returns one bounded JSON result:
