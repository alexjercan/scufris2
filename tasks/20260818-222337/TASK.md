...
- Give every delegated coding job its own Git worktree. Accept an optional concise lowercase hyphen-separated feature slug and use it exactly; otherwise use `scufris-<job_id>`. Reject collisions instead of suffixing or reusing a branch, worktree, or tmux session. Keep the opaque job ID and exact recorded tmux IDs as ownership identities. Never let concurrent coding agents share a checkout.
